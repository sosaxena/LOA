 ...
 */
class Board extends Observable implements Observer {
    ...
    public Board(int rows, int columns) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                board[i][j] = new Vector(1); //capacity of the vector is 1
            }
        }
...
//    }
    ...
    public void paint(Graphics g) {
        ...
        //Fill alterate rectangles as gray
        for(int i=0;i<rows;i+=1){
        	for(int j=(i%2+1)%2;j<columns;j+=2){
        		g.setColor(Color.GRAY);
        		g.fillRect(columnToX(j), rowToY(i),getCellWidth(),getCellHeight());
        	}
        }
